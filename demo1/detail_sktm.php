...
//ACC
if (isset($_GET['aksi']) && isset($_GET['id_request_sktm'])) {
    $id = $_GET['id_request_sktm'];
    $aksi = $_GET['aksi'];

    if ($aksi == 'acc') {
        $no_surat = "SKTM/" . date('Y') . "/" . str_pad($id, 4, '0', STR_PAD_LEFT);
        $sql = "UPDATE data_request_sktm SET status='1', no_surat='$no_surat', acc=NOW(), keterangan='Surat sudah bisa dicetak' WHERE id_request_sktm='$id'";
        $query = mysqli_query($konek, $sql);

        if ($query) {
            echo "<script>
                swal('Berhasil!', 'Pengajuan SKTM telah disetujui', 'success');
                setTimeout(function(){ window.location.href = '?halaman=detail_sktm&id_request_sktm=$id'; }, 2000);
            </script>";
        } else {
            echo "<script>
            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        <h4 class="card-title">DETAIL PENGAJUAN SURAT KETERANGAN TIDAK MAMPU</h4>
                            ...
                            <?php if ($status == "0" || $status == "ditolak") { ?>
                                <div class="row mt-3">
                                    <div class="col-md-12 text-center">
                                        <a href="?halaman=detail_sktm&id_request_sktm=<?= $id; ?>&aksi=acc"
                                        ...
                                        <button type="button" 
                                            ...
                                            onclick="openTolakModal('<?= $id; ?>', 'sktm')">
...
<div class="modal fade" id="modalTolak" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="?halaman=detail_sktm&id_request_sktm=<?= $id; ?>">
